...
function parseItemArray(v: FormDataEntryValue | null): { text: string; enabled: boolean }[] {
  const raw = String(v || '[]');
  try {
    ...
    return arr
      ...
      .map((it: any) => ({ text: String(it.text).trim(), enabled: !!it.enabled }));
  } catch {
    return [];
  }
}
...
function postFieldsFromForm(formData: FormData) {
  return {
    ...
    catchphrases: parseItemArray(formData.get('catchphrases')),
    hashtags: parseItemArray(formData.get('hashtags')),
...
export async function createPost(formData: FormData) {
  ...
  try {
    ...
    const bannerUrls = await uploadManyImages(user.id, bannerFiles);
...
export async function updatePost(postId: string, formData: FormData) {
  ...
  try {
    ...
    const keepBanners = formData.getAll('keepBannerImages') as string[];
    const newBannerFiles = formData.getAll('bannerImages') as File[];
    const newBannerUrls = await uploadManyImages(user.id, newBannerFiles);
    update.banner_images = [...keepBanners, ...newBannerUrls];
