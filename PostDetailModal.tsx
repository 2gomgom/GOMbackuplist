  ...
  return (
    <div className="fixed inset-0 bg-black/45 flex items-end sm:items-center justify-center z-[100]">
      <div className="relative w-full sm:max-w-lg max-h-[90vh] overflow-y-auto bg-white rounded-t-2xl sm:rounded-2xl p-5">
        ...
        {(post.banner_images?.length > 0 || post.banner_image_url) && (
          <div className="flex gap-2 overflow-x-auto mb-3 -mt-1" style={{ scrollbarWidth: 'none' }}>
            {(post.banner_images?.length > 0 ? post.banner_images : [post.banner_image_url]).map((url, i) => (
              <img
                key={i}
                src={url}
                alt=""
                onClick={() => setLightbox(url)}
                className="h-24 rounded-lg flex-shrink-0 cursor-pointer object-cover"
              />
            ))}
          </div>
        )}
        ...
        {(post.catchphrases?.filter(c => c.enabled && c.text).length > 0) && (
          <div className="mb-2">
            {post.catchphrases.filter(c => c.enabled && c.text).map((c, i) => (
              <p key={i} className={`text-sm text-text-muted ${post.catchphrase_style === 'italic' ? 'italic' : ''}`}>
                “{c.text}”
              </p>
            ))}
          </div>
        )}
        {(post.hashtags?.filter(t => t.enabled && t.text).length > 0) && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {post.hashtags.filter(t => t.enabled && t.text).map((t, i) => (
              <span key={i} className="text-xs font-medium text-text bg-bg-soft border border-border/60 px-2 py-0.5 rounded">
                #{t.text}
              </span>
            ))}
          </div>
